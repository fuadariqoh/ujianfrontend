...
import { HeaderNumber } from "../redux/actions";
      ...
      Axios.get(`${API_URL}/transactions?status=oncart&userId=${props.User.id}`)
        .then(res1 => {
          if (res1.data.length) {
            ...
            Axios.post(`${API_URL}/transactiondetails`, objdetails).then(
              res3 => {
                props.HeaderNumber();
                MySwal.fire({
                  icon: "success",
    ...
    return (
      <div className="paddingatas">
        <Modal centered toggle={() => setmodalopen(false)} isOpen={modalopen}>
          ...
          <ModalFooter>
            <button className="btn btn-primary" onClick={onToLoginClick}>
              OK
            </button>
          </ModalFooter>
        </Modal>
        <div className="row">
          ...
          <div className="col-md-8 p-2">
            ...
            <div className="border-headerdetail" style={{ lineHeight: "80px" }}>
              <div className="row">
                <div className="col-md-1 font-typographymed">Stok</div>
                <div className="col-md-11">{stok}pcs</div>
              </div>
            </div>
            <div
...
export default connect(MapstatetoProps, { HeaderNumber })(ProductDetail);
